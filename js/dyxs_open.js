...
async function category(tid, pg, filter, extend) {
    if (pg <= 0) pg = 1;
    const link = HOST + '/' + tid + '-' + (extend.area || '') + '-' + (extend.by || 'time') + '-' + (extend.class || '') + '-' + (extend.lang || '') + '----' + pg + '---' + (extend.year || '') + '/';//http://dyxs29.com/pianku-dianshiju-%E7%BE%8E%E5%9B%BD-hits-%E5%89%A7%E6%83%85-%E8%8B%B1%E8%AF%AD-------2022/
...
async function play(flag, id, flags) {
    ...
    const playUrl = playurl.replace(/(.*?)&d.*&.*/g, '$1');
    return JSON.stringify({
        parse: 0,
        url: playUrl,
    });
...
async function search(wd, quick, pg) {
    if (pg <= 0) pg = 1;
    let data = await request(HOST + '/search-' + wd + '-----------' + pg + '--' + '/');//https://dyxs29.com/search-%E6%88%91-----------2--/
    const $ = load(data);
